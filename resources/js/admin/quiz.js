(function ($) {
    ...
    $('body').on('click', '#add_drag_and_drop_question', function (e) {
        e.preventDefault();
        var quiz_id = $(this).attr('data-quiz-id');
        var multipleQuestionModal = $('.dragAndDropQuestionModal' + quiz_id);
        var clone = multipleQuestionModal.clone();
      ...
      $('body').on('click', '.add-question-btn', function (e) {
        ...
        var mainRow = $('.add-question-card.main-question-row').first(); // Get the main row
        var copy = mainRow.clone();
        copy.removeClass('main-question-row');
        copy.find('.answer-remove').removeClass('d-none');
        ...
        // Update the name attributes of the input fields with the new index
        copy.find('input[name^="ajax[questions]"]').each(function() {
            var newName = $(this).attr('name').replace(/\[\d+\]/, '[' + questionIndex + ']');
            $(this).attr('name', newName);
        });
    
        $('.add-question-container').append(copy);
    });
    $('body').on('click', '.answer-remove', function (e) {
      e.preventDefault();
      $(this).closest('.add-question-card').remove();
    });
    // *******************
    // edit
    // *****************
