    ...
    <div class="container">
                ...
                <div class="row">
                    <div class="col-6 col-md-3 d-flex align-items-center justify-content-center">
                        <div class="d-flex flex-column align-items-center text-center">
                            ...
                            <strong class="font-30 font-weight-bold text-secondary mt-5 passMark">{{ $quiz->pass_mark }}/{{ $quizQuestions->sum('grade') }}</strong>
        ...
        <section class="mt-30 quiz-form">
            <form action="/panel/quizzes/{{ $quiz->id }}/store-result" method="post" class="">
                ...
                <input type="hidden" id="quizQuestions" value="{{ json_encode($quizQuestions) }}">
                <input type="hidden" name="total_mark" value="{{ $quizQuestions->sum('grade') }}">               
                    @foreach($quizQuestions as $key => $question)
                        <fieldset class="question-step question-step-{{ $key + 1 }}">
                            <div class="rounded-lg shadow-sm py-25 px-20">
                                <div class="quiz-card">
                                <input type="hidden" name="quiz_type" value="{{ $question->type }}"> 
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <ul class="card-list question-list" id="question-list-{{ $key }}">
                                                @foreach($question->translations->shuffle() as $translation)
                                                <input type="hidden" name="question[{{ $question->id }}][{{ $translation->id }}]" value="{{ $translation->title }}">
                                                    <li class="card question-translation" data-id="{{ $translation->id }}">{{ $translation->title }}</li>
                                                @endforeach
                                            </ul>
                                        </div>
                                        <div class="col-md-6">
                                            <ul class="card-list answer-list" id="answer-list-{{ $key }}">
                                                @foreach($question->quizzesQuestionsAnswers->shuffle() as $answer)
                                                    <li class="card" data-id="{{ $answer->id }}">{{ $answer->title }}</li>
                                                @endforeach
                                            </ul>
                                        </div>
                                    </div>
