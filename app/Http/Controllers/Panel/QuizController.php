...
class QuizController extends Controller
{
        ...
        if ($quiz) {
            ...
            if (!isset($quiz->attempt) or ($userQuizDone->count() < $quiz->attempt and !$status_pass)) {
                ...
                $quizQuestionsQuery = QuizzesQuestion::query()
                    ...
                    $res = QuizzesQuestion::where('quiz_id',$quiz->id)->get('type');
                   
                 

                    if ($res->contains('type', 'dragAndDrop')) {
    ...
    public function quizzesStoreResult(Request $request, $id)
    {
        ...
        $totalMark = $request->input('total_mark');
        ...
        if ($quiz) {
            ...
            $results = $request->input('question');
            if ($quizType === QuizzesQuestion::$dragAndDrop) {
                $totalMark = 0; 
                foreach ($results as $questionId => $answers) {
                    $results2 = QuizzesQuestion::where('id', $questionId)->first();
                   $totalMark += $results2->grade; 
                }
            }else{
    ...
    public function myResults(Request $request)
    {
        try {
            ...
            $data = [
                ...
                'quizzesResults' => $quizResults, // Correct variable name
            ...
            \Log::error('Error in myResults function: ' . $e->getMessage());
